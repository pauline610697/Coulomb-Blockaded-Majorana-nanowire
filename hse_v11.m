function H = hse_v11(t,Delta1,Delta2,N_tot,alpha,mu,VD1,VD2,N_dot,Nbarrier,Ebarrier,Vz,lambda,omega)
...
%SelfE = -(lambda./2).*((omega*t0s0 + Delta1*txs0)./sqrt(Delta1.^2 -omega.^2) + (omega*t0s0 + Delta2*txs0)./sqrt(Delta2.^2 -omega.^2));
SelfE = -lambda.*(0.1*(omega*t0s0 + Delta1*txs0)./sqrt(Delta1.^2 -omega.^2) + 0.9*(omega*t0s0 + Delta2*txs0)./sqrt(Delta2.^2 -omega.^2));
...
%H = kron(eyesm, (2*t - mu)*tzs0 + Vz*t0sx) + kron(barrier,Ebarrier*tzs0)+ kron(dot1, tzs0) + kron(NOT_dot, Delta1*txs0) + kron(dot2,tzs0);
